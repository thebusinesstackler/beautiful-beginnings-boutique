...
const EmbeddedSquareCheckout = memo(({
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      try {
        ...
        const uploadedImages = cartItems.filter(item => item.uploadedPhotoUrl).map(item => item.uploadedPhotoUrl!).filter(Boolean);
      ...
      if (result.status !== 'OK') {
        ...
        setPaymentResult({
          success: false,
          message: 'Failed to tokenize card. Please check your details.'
        });
        setIsPaying(false);
        return;
      }
      ...
      console.log('📊 Square payment function response:', {
        data,
        error
      });
      if (error) {
        ...
        setPaymentResult({
          success: true,
          message: `Payment successful! Order: ${orderId.substring(0, 8)}...`
        });
      ...
      setPaymentResult({
        success: false,
        message: err.message || 'Unexpected error during payment.'
      });
      onError?.(err);
    } finally {
      setIsPaying(false);
  ...
  return <div className="bg-white rounded-xl border border-sage/20 shadow-sm">
      {/* Header */}
      ...
      {/* Content */}
      <div className="p-6">
        {sdkStatus === 'loading' && <div className="text-center py-12">
        ...
        {sdkStatus === 'error' && <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        ...
        {sdkStatus === 'ready' && <div className="space-y-4">
            ...
            <div className="relative">
              <div ref={cardRef} id="sq-card-container" style={{
            minHeight: '80px'
          }} className="w-full border border-gray-300 rounded-lg p-4 bg-white min-h-[80px] focus-within:ring-2 focus-within:ring-sage focus-within:border-sage mx-0 my-[4px] px-0 py-0" />
              
              {/* Loading overlay for card container */}
              {cardRef && !card && <div className="absolute inset-0 bg-white/80 rounded-lg flex items-center justify-center">
            ...
            <button onClick={handlePayment} disabled={isPaying || !card || !cardRef?.current || !isFormValid} className="w-full bg-sage text-white py-3 px-4 rounded-lg hover:bg-sage/90 transition disabled:opacity-50 disabled:cursor-not-allowed font-medium">
              {!isFormValid ? 'Complete Required Fields First' : isPaying ? <div className="flex items-center justify-center">
                  <div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent mr-2"></div>
                  Processing Payment...
                </div> : `Complete Payment - $${total?.toFixed(2) || '0.00'}`}
            </button>

            {paymentResult && <div className={`p-3 rounded-lg ${paymentResult.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
