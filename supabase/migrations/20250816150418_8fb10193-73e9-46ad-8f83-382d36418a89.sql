-- Fix orders table security - handle existing data first

-- Step 1: Handle existing orders with null customer_id
-- Option 1: Delete orders with null customer_id (safer for security)
DELETE FROM public.orders WHERE customer_id IS NULL;

-- Step 2: Make customer_id not nullable
ALTER TABLE public.orders ALTER COLUMN customer_id SET NOT NULL;

-- Step 3: Drop all existing problematic email-based policies
...
-- Step 4: Update the trigger to set customer_id to auth.uid() for authenticated users
...
-- Step 5: Create strict RLS policies based on customer_id and auth.uid()
...
-- Admins can manage all orders (this takes precedence over user policies)
